import React from "react";

const DeleteNote = (props) => {
    const deleteNote = (id,e) => {
      //Get current notes
      const notes = localStorage.getItem('notes').split("\n");
      //Write notes withput the note to delete.
      localStorage.setItem('notes', notes.filter(note => note.split("|")[2]!=id).join("\n"));
    }
    return (
      <span className="float-right mr-3" title="Eliminar" role="button" value={props.time} onClick={(e) => deleteNote(props.time, e)}>
        <svg width="1em" height="1em" viewBox="0 0 16 16" className="bi bi-trash-fill" fill="currentColor">
          <path d="M2.5 1a1 1 0 0 0-1 1v1a1 1 0 0 0 1 1H3v9a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2V4h.5a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1H10a1 1 0 0 0-1-1H7a1 1 0 0 0-1 1H2.5zm3 4a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 .5-.5zM8 5a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7A.5.5 0 0 1 8 5zm3 .5a.5.5 0 0 0-1 0v7a.5.5 0 0 0 1 0v-7z"/>
        </svg>
      </span>
    )
  }

  export default DeleteNote;